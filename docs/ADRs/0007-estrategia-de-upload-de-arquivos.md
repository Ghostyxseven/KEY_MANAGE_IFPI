...
Decision
--------
Adotaremos **Supabase Storage** (buckets) para armazenar meios e arquivos durante o MVP, usando URLs assinadas (presigned/signed URLs) ou uploads diretos quando suportado pelo cliente.

Consequences
------------
- Integrar SDK da Supabase para geração de URLs assinadas e operações de upload/download.
...
--------------------
- Use the Supabase Storage SDK to create buckets and manage object ACLs. For public assets a public bucket is acceptable; for private assets generate signed URLs server-side using the service role key.
...
- Environment variable: `SUPABASE_SERVICE_ROLE_KEY` required for server operations that generate signed URLs or manage private buckets.
